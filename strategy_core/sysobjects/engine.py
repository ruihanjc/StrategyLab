...
class BacktestEngine(TradingEngine):
    ...
    def _save_signals_to_file(self, portfolio_results: pd.DataFrame):
        ...
        signals_file = os.path.join(results_dir, f"signals_{timestamp}.csv")
        ...
        try:
            with open(signals_file, 'w', newline='') as f:
                ...
                for instrument in self.portfolio.instruments:
                    ticker = instrument.ticker
                    header.extend([f"{ticker}_Signal", f"{ticker}_Position"])
                header.extend(["Portfolio_Value"])
                writer.writerow(header)
                
                # Write signals data - only record when positions change
                prev_positions = {instrument.ticker: 0.0 for instrument in self.portfolio.instruments}
                ...
                for date, row in portfolio_results.iterrows():
                    ...
                    if position_changed or date == portfolio_results.index[0] or date == portfolio_results.index[-1]:
                        ...
                        for instrument in self.portfolio.instruments:
                            ...
                            row_data.extend([signal_str, f"{position_val:.4f}"])
                        
                        portfolio_val = row.get('portfolio_value', 0.0)
                        row_data.append(f"{portfolio_val:.2f}")
    ...
    def _save_signals_to_file(self, portfolio_results: pd.DataFrame):
        ...
        try:
            ...
            for instrument in self.portfolio.instruments:
                ticker = instrument.ticker
                header.extend([f"{ticker}_Signal", f"{ticker}_Position"])
            header.extend(["Portfolio_Value", "Generated"])
                ...
                with open(signals_file, 'w', newline='') as f:
                    ...
                    for date, row in portfolio_results.iterrows():
                        ...
                        if position_changed or date == portfolio_results.index[0] or date == portfolio_results.index[-1]:
                            ...
                            row_data.extend([f"{portfolio_val:.2f}", timestamp])
                            writer.writerow(row_data)
